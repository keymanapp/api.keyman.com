<?php
  // Mimic the KeymanWeb Cloud json 2.0 API but from our t_keyboards data
  ...
  if($context == 'language') {
    if(empty($languageid)) {
      $response['languages'] = getLanguages($languageid);
    } else {
      $response['language'] = getLanguages($languageid);
    }
  } else if($context == 'keyboard') {
    if(empty($keyboardid)) {
      $response['keyboards'] = getKeyboards($keyboardid, $languageid);
    } else {
...
*/
  function getKeyboardInfo($keyboard, $languageid, $includeVersion) {
    global $device;
  ...
  function getKeyboards($keyboardid, $languageid) {
    if(empty($languageid) && empty($keyboardid)) {
      ...
      $keyboards = DB_LoadKeyboards($keyboardid);
      if(sizeof($keyboards) > 0) {
        return getKeyboardInfo($keyboards[0], $languageid, false);
      }
      fail("Keyboard not found", 404);
    }
